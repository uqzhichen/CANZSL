...
parser.add_argument('--resume', default='./out/Best_model_ZSL_Acc_56.49.tar',  type=str, help='the model to resume') #./out/Best_model_ZSL_Acc_51.64.tar
...
def train():
    ...
    start_step = 0
    G2D2_checkpoint = torch.load("./out/G2D2.tar")
    netD2.load_state_dict(G2D2_checkpoint['state_dict_D2'])
    netG2.load_state_dict(G2D2_checkpoint['state_dict_G2'])
    netG2.eval()
    if opt.resume:
    ...
    for it in range(start_step, 20000+1):
        """ Discriminator """
        for _ in range(5):
        ...
        if it % opt.disp_interval == 0 and it:
            ...
            log_text = 'Iter-{}; Was_D: {:.3f};  Euc_ls: {:.3f}; reg_ls: {:.3f}; \n' \
                       'G_loss: {:.3f}; D_loss_real: {:.3f}; D_loss_fake: {:.3f};' \
                       ' rl: {:.2f}%; fk: {:.2f}%;cycle: {:.3f} \n'\
                        .format(it, Wasserstein_D.item(),  Euclidean_loss.item(),
                                reg_loss.item(), G_loss.item(), D_loss_real.item(),
                                D_loss_fake.item(),
