...
from models import _netD, _netG, _netD2, _netG2, _param

parser = argparse.ArgumentParser()
parser.add_argument('--gpu', default='1', type=str, help='index of GPU to use')
parser.add_argument('--dataset', default='CUB2011', type=str, help='dataset to experiment: NABird or CUB2011')
parser.add_argument('--splitmode', default='easy', type=str, help='the way to split train/test data: easy/hard')
parser.add_argument('--manualSeed', type=int, help='manual seed')
parser.add_argument('--resume', default='', type=str,
                    help='the model to resume')  # ./CUB_checkpoints/Best_model_Acc_43.68.tar
...
parser.add_argument('--cyclelambda',  type=float, default=10)
...
def train():
    ...
    param.text_dim = dataset.text_dim
    ...
    netG2 = _netG2(dataset.feature_dim).cuda()
    netG2.apply(weights_init)
    print(netG2)
    netD2 = _netD2(dataset.train_cls_num).cuda()
    ...
    exp_params = 'Eu{}_Rls{}_RWz{}_cyc{}_cla{}_lr{}_Knn{}_nS{}_b{}'.format(opt.CENT_LAMBDA, opt.REG_W_LAMBDA,
                                                 opt.REG_Wz_LAMBDA, opt.cyclelambda, opt.clalambda,
                                                                  opt.lr, opt.Knn, opt.nSample, opt.batchsize)
    ...
    for it in range(start_step, 4000 + 1):
        ...
        for _ in range(5):
            ...
            # G1 results
            visual_sample, rdc_text = netG(z, text_feat)

            # real loss
            D2_real, C2_real = netD2(rdc_text)
            D2_loss_real = torch.mean(D2_real)
            C2_loss_real = F.cross_entropy(C2_real, y_true)
            DC2_loss = -D2_loss_real + opt.clalambda * C2_loss_real
            DC2_loss.backward()

            # fake loss
            rdc_text_sample = netG2(z2, visual_sample).detach()
            D2_fake, C2_fake = netD2(rdc_text_sample)
            D2_loss_fake = torch.mean(D2_fake)
            C2_loss_fake = F.cross_entropy(C2_fake, y_true)
            DC2_loss = D2_loss_fake + opt.clalambda * C2_loss_fake
            DC2_loss.backward()

            # train with gradient penalty (WGAN_GP)
            grad_penalty2 = calc_gradient_penalty(netD2, rdc_text.data, rdc_text_sample.data)
        ...
        for _ in range(1):
            ...
            _, rdc_text = netG(z, text_feat)
            rdc_text_sample = netG2(z2, X)
            D2_fake, C2_fake = netD2(rdc_text_sample)
            _, C2_real = netD2(rdc_text)
            ...
            GC2_loss = -G2_loss + opt.clalambda * C2_loss
        ...
        for _ in range(1):
            ...
            G_sample, rdc_text = netG(z, text_feat)
            rdc_text_sample = netG2(z2, G_sample)

            cycle_loss = opt.cyclelambda*torch.nn.MSELoss()(rdc_text, rdc_text_sample)
            cycle_loss.backward()
        ...
        if it % opt.disp_interval == 0 and it:
            ...
            log_text = 'Iter-{}; Was_D: {:.4}; VP_ls: {:.4}; reg_ls: {:.4}; Wz_ls: {:.4}; G_loss: {:.4}; ' \
                       'D_loss_real: {:.4}; D_loss_fake: {:.4}; \n' \
                    'Was_D2: {:.4}; reg_ls2: {:.4};  G2_loss: {:.4}; ' \
                    'D2_loss_real: {:.4}; D2_loss_fake: {:.4}; rl: {:.4} %; fk: {:.4} % \n'\
                .format(it, Wasserstein_D.item(), Euclidean_loss.item(), reg_loss.item(), reg_Wz_loss.item(),
                        ...
                        D2_loss_fake.item(),acc_real * 100, acc_fake * 100)
            print(log_text)
            with open(log_dir, 'a') as f:
                f.write(log_text + '\n')
